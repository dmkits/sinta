 ...
 */
function doDialogMsg(params, onExecute, onCancel) {
    require(["dijit/registry", "dijit/ConfirmDialog", "dijit/form/Button", "dojo/domReady!"],
        function (registry, ConfirmDialog, Button) {
            var myDialog = initElem(registry, "DialogSimple", null, ConfirmDialog, {});
            if (params.title) myDialog.set("title", params.title);
            if (params.content) myDialog.set("content", params.content);
 ...
 * @param onCancel
 */
function doDialogSimpleTextBox(params, onExecute, onCancel) {
    require(["dijit/registry", "dijit/ConfirmDialog", "dijit/form/TextBox", "dojo/domReady!"], function (registry, ConfirmDialog, TextBox) {
        var myDialog = initElem(registry, "DialogSimpleTextBox", null, ConfirmDialog, {});
        if (params.title) myDialog.set("title", params.title);
        if (params.style) myDialog.set("style", params.style);
        if (params.btnOkLabel) myDialog.set("buttonOk", params.btnOkLabel);
        if (params.btnCancelLabel) myDialog.set("buttonCancel", params.btnCancelLabel);
        ...
        for (var item in content) {
            if (content[item].type == "textbox") {
                ...
                var dialogtextbox = initElem(registry, content[item].name, content[item].name, TextBox, {type: "text"});
                ...
                if (width) dialogtextbox.set("style", "width:" + width);
        ...
        if (onExecute != null) myDialog.onExecute = function () {
            registredAction(function (registry) {
                var content = params.content;
                for (var item in content) {
                    if (content[item].type == "textbox") {
                        content[item].value = registry.byId(content[item].name).value;
                    }
                }
 ...
 * @param onCancel
 */
function doDialogSimpleTree(params, onExecute, onCancel) {
 ...
 * @param onCancel
 */
function doDialogSimpleList(params, onInit, onExecute, onCancel) {
    require(["dijit/registry", "dijit/ConfirmDialog",
            "dijit/layout/ContentPane", "dijit/layout/BorderContainer", "htable", "dojo/domReady!"],
        function (registry, ConfirmDialog, ContentPane, BorderContainer, HTable) {
...
//function doDialog1(title, content, style, onExecute, onCancel) {
...
function mainAboutDialog(){
    doDialogMsg({title:"О программе",content:"Система учета <b>BATA</b> для магазина обуви. <br>Разработчик: dmkits 2016",
